...
#     def generate_path():
#         for _ in range(50):  # attempt multiple times
#             path = [source]
#             visited = set(path)
#             while path[-1] != dest:
#                 current = path[-1]
...
#                 dest = edges[-1].lstrip('-')
#                 vehicle_data.append((vid, source, dest))
#     return vehicle_data
...
#         next_t = t + 300
#         print(f"\n⏱️ Simulation time: {t}s ➜ Predicting for {next_t}s")

#         try:
#             densities = load_density_at_time("../simulation_data/runtime_routes.csv", next_t)
#         except ValueError:
#             print(f"⚠️ No prediction for {next_t}s. Stopping.")
#             break
...
#                 print(f"❌ Invalid junctions for {vehicle_id}")
#                 continue
#             if not path_exists(junction_graph, source_j, dest_j):
#                 print(f"❌ No raw path between {source_j} and {dest_j} for {vehicle_id}")
#                 continue

#             path, cost = run_pso(weighted_graph, source_j, dest_j)
#             if path:
#                 print(f"✅ PSO Path for {vehicle_id} at {t}s: {path} | Cost: {cost:.2f}")
#             else:
#                 print(f"❌ PSO failed for {vehicle_id} at {t}s")
