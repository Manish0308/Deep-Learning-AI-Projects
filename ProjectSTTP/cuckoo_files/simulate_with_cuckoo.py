...
def generate_path(weighted_graph, source, dest):
    for _ in range(50):
        path = [source]
        visited = set(path)
        while path[-1] != dest:
...
def fitness(path, graph):
    return sum(graph[path[i]].get(path[i+1], float('inf')) for i in range(len(path) - 1))

def run_cuckoo_search(graph, source, dest, n=25, iterations=50, pa=0.25):
    nests = [generate_path(graph, source, dest) for _ in range(n)]
    nests = [p for p in nests if p]
    if not nests:
        return None, float('inf')

    best_nest = min(nests, key=lambda p: fitness(p, graph))
    best_cost = fitness(best_nest, graph)

    for _ in range(iterations):
        for i in range(len(nests)):
            new_path = generate_path(graph, source, dest)
            if new_path and fitness(new_path, graph) < fitness(nests[i], graph):
                nests[i] = new_path

        if random.random() < pa and nests:
            idx = random.randint(0, len(nests) - 1)
            replacement = generate_path(graph, source, dest)
            if replacement:
                nests[idx] = replacement

        current_best = min(nests, key=lambda p: fitness(p, graph))
        current_cost = fitness(current_best, graph)
        if current_cost < best_cost:
            best_nest, best_cost = current_best, current_cost

    return best_nest, best_cost
...
def main():
    ...
    param_combinations = [
        (10, 20, 0.1), (10, 40, 0.1), (10, 60, 0.2), (10, 80, 0.2), (10, 100, 0.3),
        (20, 20, 0.1), (20, 40, 0.2), (20, 60, 0.2), (20, 80, 0.3), (20, 100, 0.3)
    ]

    for idx, (n, iterations, pa) in enumerate(param_combinations, start=1):
        output_csv = f"cuckoo_rerouted_paths_trial{idx}.csv"
        output_xml = f"cuckoo_final_edge_route_trial{idx}.xml"
        final_routes = []
        ...
        with open(output_csv, 'w', newline='') as csvfile:
            ...
            for t in range(0, 14400, 300):
                ...
                for vehicle_id, source_edge, dest_edge, depart in vehicle_routes:
                    ...
                    path, cost = run_cuckoo_search(weighted_graph, source_j, dest_j,
                                                   n=n, iterations=iterations, pa=pa)
                    if path:
...
#     distance_file = "../data/aligned_normalized_distance_matrix.csv"
#     density_file = "../simulation_data/runtime_routes.csv"
#     output_csv = "cuckoo_rerouted_paths.csv"
#     output_xml = "cuckoo_final_edge_route.xml"
...
#     # Define 10 different parameter sets: (num_nests, max_generations)
#     param_combinations = [
#         (10, 20), (10, 40), (10, 60), (10, 80), (10, 100),
#         (20, 20), (20, 40), (20, 60), (20, 80), (20, 100)
#     ]

#     for idx, (num_nests, max_generations) in enumerate(param_combinations, start=1):
#         output_csv = f"cuckoo_rerouted_paths_trial{idx}.csv"
#         output_xml = f"cuckoo_final_edge_route_trial{idx}.xml"
#         final_routes = []
...
#                     path, cost = run_cuckoo_search(weighted_graph, source_j, dest_j,
#                                                    num_nests=num_nests, max_generations=max_generations)
#                     if path:
#                         edge_route = convert_junction_path_to_edge_route(path, edge_map)
#                         if edge_route:
#                             writer.writerow([t, vehicle_id, ' '.join(edge_route)])
