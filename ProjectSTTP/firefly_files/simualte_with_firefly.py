...
# def run_firefly(weighted_graph, source, dest, num_fireflies=20, num_iterations=50, gamma=1.0, beta0=1.0):
#     def fitness(path):
...
#     fireflies = [generate_path() for _ in range(num_fireflies)]
#     fireflies = [f for f in fireflies if f]
...
#                 if i != j:
#                     cost_i = fitness(fireflies[i])
#                     cost_j = fitness(fireflies[j])
#                     if cost_j < cost_i:
#                         dist = len(set(fireflies[i]) ^ set(fireflies[j]))
#                         beta = beta0 * pow(2.71828, -gamma * dist)
...
#     output_csv = "firefly_rerouted_paths.csv"
#     output_xml = "firefly_final_edge_route.xml"
...
#                 path, cost = run_firefly(weighted_graph, source_j, dest_j)
#                 if path:
...
def main():
    ...
    for idx, (num_fireflies, num_iterations, gamma, beta0) in enumerate(param_combos):
        output_csv = f"firefly_rerouted_paths_{idx+1}.csv"
        output_xml = f"firefly_final_edge_route_{idx+1}.xml"
        final_routes = []
        ...
        write_final_routes(final_routes, output_xml)
        print(f"✅ Firefly run {idx+1} completed → XML: {output_xml}")
